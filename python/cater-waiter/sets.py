...
def categorize_dish(dish_name, dish_ingredients):
    ...
    if VEGETARIAN >= dish_ingredients:
        return dish_name + ": VEGETARIAN"
    if PALEO >= dish_ingredients:
        return dish_name + ": PALEO"
    if KETO >= dish_ingredients:
        return dish_name + ": KETO"
    if OMNIVORE >= dish_ingredients:
...
def separate_appetizers(dishes, appetizers):
    ...
    dish_set = set(dishes)
    apps_set = set(appetizers)

    return list(dish_set.difference(apps_set))
